...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('smoConnectFrontendApp.error', (response: EventWithContent<unknown> | string) => {
    ...
    this.httpErrorListener = this.eventManager.subscribe(
      'smoConnectFrontendApp.httpError',
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(`smoConnectFrontendApp.${fieldError.objectName as string}.${convertedField}`);
