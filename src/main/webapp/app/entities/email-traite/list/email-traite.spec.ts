...
import { sampleWithRequiredData } from '../email-traite.test-samples';
import { EmailTraiteService } from '../service/email-traite.service';

import { EmailTraite } from './email-traite';

vitest.useFakeTimers();

describe('EmailTraite Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: EmailTraite;
  let fixture: ComponentFixture<EmailTraite>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(EmailTraite);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 18838 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.emailTraites()[0]).toEqual(expect.objectContaining({ id: 18838 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 18838 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.emailTraites()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 18838 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to emailTraiteService', () => {
      const entity = { id: 18838 };
      vitest.spyOn(service, 'getEmailTraiteIdentifier');
      const id = comp.trackId(entity);
      expect(service.getEmailTraiteIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.emailTraitesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
