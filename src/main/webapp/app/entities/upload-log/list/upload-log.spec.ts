...
import { sampleWithRequiredData } from '../upload-log.test-samples';

import { UploadLog } from './upload-log';

vitest.useFakeTimers();

describe('UploadLog Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: UploadLog;
  let fixture: ComponentFixture<UploadLog>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(UploadLog);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 31502 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.uploadLogs()[0]).toEqual(expect.objectContaining({ id: 31502 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 31502 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.uploadLogs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 31502 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to uploadLogService', () => {
      const entity = { id: 31502 };
      vitest.spyOn(service, 'getUploadLogIdentifier');
      const id = comp.trackId(entity);
      expect(service.getUploadLogIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.uploadLogsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
