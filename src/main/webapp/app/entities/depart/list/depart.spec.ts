...
import { sampleWithRequiredData } from '../depart.test-samples';
...
describe('Depart Management Component', () => {
  ...
  let comp: Depart;
  let fixture: ComponentFixture<Depart>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Depart);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 1161 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.departs()[0]).toEqual(expect.objectContaining({ id: 1161 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 1161 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.departs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 1161 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to departService', () => {
      const entity = { id: 1161 };
      vitest.spyOn(service, 'getDepartIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.departsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
