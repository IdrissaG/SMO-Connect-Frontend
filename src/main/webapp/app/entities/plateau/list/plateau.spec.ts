...
import { sampleWithRequiredData } from '../plateau.test-samples';
...
describe('Plateau Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 29046 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.plateaus()[0]).toEqual(expect.objectContaining({ id: 29046 }));
  });

  describe('trackId', () => {
    it('should forward to plateauService', () => {
      const entity = { id: 29046 };
      vitest.spyOn(service, 'getPlateauIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.plateausParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
