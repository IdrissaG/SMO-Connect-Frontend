...
import { sampleWithRequiredData } from '../effectif.test-samples';
...
describe('Effectif Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Effectif;
  let fixture: ComponentFixture<Effectif>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Effectif);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 14359 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.effectifs()[0]).toEqual(expect.objectContaining({ id: 14359 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 14359 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.effectifs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 14359 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to effectifService', () => {
      ...
      vitest.spyOn(service, 'getEffectifIdentifier');
      const id = comp.trackId(entity);
      expect(service.getEffectifIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.effectifsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
