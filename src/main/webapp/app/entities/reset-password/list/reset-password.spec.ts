...
import { sampleWithRequiredData } from '../reset-password.test-samples';
...
describe('ResetPassword Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ResetPassword;
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 13864 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.resetPasswords()[0]).toEqual(expect.objectContaining({ id: 13864 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 13864 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.resetPasswords()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 13864 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to resetPasswordService', () => {
      ...
      vitest.spyOn(service, 'getResetPasswordIdentifier');
      const id = comp.trackId(entity);
      expect(service.getResetPasswordIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.resetPasswordsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
