...
import { sampleWithRequiredData } from '../prestataire.test-samples';
...
describe('Prestataire Management Component', () => {
  ...
  let comp: Prestataire;
  let fixture: ComponentFixture<Prestataire>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Prestataire);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 27618 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.prestataires()[0]).toEqual(expect.objectContaining({ id: 27618 }));
  });

  describe('trackId', () => {
    it('should forward to prestataireService', () => {
      const entity = { id: 27618 };
      vitest.spyOn(service, 'getPrestataireIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPrestataireIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.prestatairesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
