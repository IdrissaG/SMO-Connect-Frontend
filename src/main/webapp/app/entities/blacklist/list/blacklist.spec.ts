...
import { sampleWithRequiredData } from '../blacklist.test-samples';
...
describe('Blacklist Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 22416 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.blacklists()[0]).toEqual(expect.objectContaining({ id: 22416 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 22416 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.blacklists()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 22416 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to blacklistService', () => {
      const entity = { id: 22416 };
      vitest.spyOn(service, 'getBlacklistIdentifier');
      const id = comp.trackId(entity);
      expect(service.getBlacklistIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.blacklistsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
